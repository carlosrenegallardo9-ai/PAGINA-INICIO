...
export function updateTelemetry() {
    ...
    document.getElementById('apoapsis').textContent = state.apoapsis === Infinity ? 'ESCAPE' : (state.apoapsis / 1000).toFixed(1) + ' km'; // ESCAPE: fuga hiperbólica
    ...
    if (guidanceInfo) {
        if (state.guidanceDebug && state.guidancePhase === 'vacuum-guidance' && state.engineOn) {
            const strategy = state.guidanceDebug.useDirectAscent ? 'Ascenso Directo' : 'Tradicional';
            const burnType = state.guidanceIsRetrograde ? 'RETRÓGRADO' : 'PROGRADO';
            guidanceInfo.textContent = `Estrategia: ${strategy} | Quemado: ${burnType}`;
    ...
    if (state.burnMode && state.burnStartTime !== null) {
        ...
        burnStatus.innerHTML = `
            <div class="burn-status-active">QUEMANDO: ${burnNames[state.burnMode]}</div>
            <div class="burn-status-duration">Duración: ${burnDuration.toFixed(1)}s</div>
        `;
    } else {
        burnStatus.innerHTML = '<div>Mantén el botón para quemar</div>';
    }
...
function updateStructuralPanel() {
    ...
    if (!sections) {
        panel.innerHTML = '<div class="structural-no-data">Lanza para ver los datos estructurales</div>';
        return;
    }
    ...
    for (const s of sections) {
        ...
        html += `<span class="structural-util-label" style="color:${color}">${pct}% del límite</span>`;
        html += `</div>`;
        ...
        html += `<div class="structural-row"><span>Aro</span><span>${fmtMPa(s.hoopStress)}</span></div>`;
        html += `<div class="structural-row"><span>Axial</span><span>${fmtMPa(s.axialStress)}</span></div>`;
        html += `<div class="structural-row"><span>Flexión</span><span>${fmtMPa(s.bendingStress)}</span></div>`;
        html += `<div class="structural-row"><span>Cizalla</span><span>${fmtMPa(s.shearStress)}</span></div>`;
        html += `<div class="structural-row structural-vm"><span>Von Mises</span><span style="color:${color}">${fmtMPa(s.vonMises)}</span></div>`;
        html += `<div class="structural-row structural-yield-row"><span>Límite</span><span>${fmtMPa(s.yieldStress)}</span></div>`;
        html += `</div>`;
        ...
        if (overYield) {
            ...
            const remaining = state.settings.structuralFailureMode === 'terminate' && state.structuralFailureTime !== null
                ...
                : null;
            html += `<div class="structural-warn">${remaining !== null ? `ROTURA EN ${remaining}s` : 'CEDIENDO'}</div>`;
        }
    ...
    if (!html) {
        panel.innerHTML = '<div class="structural-no-data">Sin etapas activas</div>';
        return;
    }
