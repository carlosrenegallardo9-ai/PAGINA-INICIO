...
function syncEventsPanelMode() {
    ...
    collapseBtn.title = state.eventsCollapsed ? 'Expandir eventos de misión' : 'Minimizar eventos de misión';
}
        ...
        if (!state.engineOn && isAscending && altitudeToApoapsis > 0) {
            ...
            if (timeUntilBurnStart > 0 && timeUntilBurnStart < 10000) {
                events.push({ 
                    ...
                    name: 'Inicio de quemado de circularización',
    ...
    if (periError >= -tolerance && apoError > tolerance) {
        ...
        if (absoluteRetrogradeBurnStartTime !== null) {
            ...
            if (timeUntilBurnStart > 0 && timeUntilBurnStart < 10000) {
                events.push({ 
                    time: timeUntilBurnStart, 
                    name: 'Inicio de quemado retrógrado',
                    type: 'retrograde',
                    burnTime: 0,
                    deltaV: 0
                });
...
export function getNextEvent() {
    ...
    if (state.time < 10 && (state.gameMode === 'manual' || state.gameMode === 'guided')) {
        events.push({ time: 10 - state.time, name: 'Inicio del programa de cabeceo' });
    }
    ...
    if (altitude < KARMAN_LINE && !state.events.some(e => e.text.includes("Kármán"))) {
        ...
        if (vVert > 0) {
            ...
            if (timeToKarman > 0 && timeToKarman < 10000 && isFinite(timeToKarman)) {
                events.push({ time: timeToKarman, name: 'Línea de Kármán' });
            }
        }
    }
    ...
    if (!state.fairingJettisoned && altitude < getRocketConfig().fairingJettisonAlt) {
        ...
        if (vVert > 0) {
            ...
            if (timeToFairing > 0 && timeToFairing < 10000 && isFinite(timeToFairing)) {
                events.push({ time: timeToFairing, name: 'Eyección del carenado' });
            }
        }
    }
    ...
    if (state.currentStage === 0 && state.propellantRemaining[0] > 0 && state.engineOn) {
        ...
        if (massFlowRate > 0) {
            ...
            if (timeToSeparation > 0 && timeToSeparation < 10000) {
                events.push({ time: timeToSeparation, name: 'Separación de etapa' });
            }
        }
    }
    ...
    if (!inOrbit && altitude < 150000) {
        ...
        if (vVert > 0) {
            ...
            if (timeToOrbit > 0 && timeToOrbit < 10000 && isFinite(timeToOrbit)) {
                ...
                events.push({ time: totalTime, name: 'Órbita' });
            }
        }
    }
