...
const SEGMENT_LABELS = ['E1', 'E2', 'CU', 'CN'];
 ...
 */
const SNAP_SEGMENT_NAMES = ['E1', 'E2', 'CU', 'CN'];
...
function drawDiameterSnapGuide(ctx, segments, snapState, scale, centerX) {
    ...
    ctx.fillText(`AJUSTE · Ø ${snapState.value.toFixed(1)} m (${fromName} = ${toName})`, centerX, yTop - 4);
    ctx.restore();
}
...
export function drawHangarRocket(canvas, config, uiState) {
    ...
    ctx.fillText(`Acel. en plataforma: ${accelG.toFixed(2)} g`, cw - pad, pad + 10);
