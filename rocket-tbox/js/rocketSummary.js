...
export function updateRocketSummary(root = document) {
    ...
    const readinessLabel = readiness < 40 ? 'CRÍTICO' : readiness < 65 ? 'MARGINAL' : readiness < 88 ? 'NOMINAL' : 'ÓPTIMO';
    ...
    if (titleEl) titleEl.textContent = config.name || (custom ? 'COHETE PERSONALIZADO' : 'FALCON-9 MK1');
    if (metaEl) metaEl.textContent = `${config.stages.length} ETAPAS · ${config.propellantType || 'KEROLOX'}`;
    ...
    let warnText = '';
    if (twr < 1.2) warnText = `El TWR de ${twr.toFixed(2)} puede ser insuficiente para despegar. Reduce la masa de carga útil o aumenta el empuje de la Etapa 1.`;
    else if (totalDV < 8000) warnText = `El ΔV estimado de ${(totalDV / 1000).toFixed(1)} km/s está por debajo del mínimo para LEO (~9.4 km/s). Añade propelente o reduce la masa seca.`;
