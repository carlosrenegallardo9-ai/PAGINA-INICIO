...
function drawForceDiagramToCanvas(targetCanvas) {
    ...
    // Legend — bottom left
    const legend = [
        { color: '#f87171', label: 'G Gravedad' },
        { color: '#4ade80', label: 'T Empuje' },
        { color: '#fbbf24', label: 'D Arrastre' },
...
function drawForceDiagram(ctx, canvas, expanded = false) {
    ...
    ctx.fillText('FUERZAS' + (expanded ? ' (Clic para cerrar)' : ' (Clic para expandir)'), centerX, centerY - diagramSize/2 + (expanded ? 18 : 12));
...
function drawRocketDiagram(ctx, canvas, expanded = false) {
    ...
    ctx.font = expanded ? '14px Courier New' : '10px Courier New';
    ctx.textAlign = 'center';
    ctx.fillText('COHETE' + (expanded ? ' (Clic para cerrar)' : ' (Clic para expandir)'), centerX, centerY - diagramHeight/2 + (expanded ? 18 : 12));
