...
export function calculateStructuralLoads() {
    ...
    if (state.currentStage === 0) {
        ...
        sections.push({ name: 'Interetapa', active: false, utilization: 0 });
        sections.push({ name: 'Base E1',    active: false, utilization: 0 });
    } else {
        sections.push({ name: 'Base E1',  active: false, utilization: 0 });
        sections.push({ name: 'Interetapa', active: false, utilization: 0 });
        sections.push({ name: 'Base E2',  active: false, utilization: 0 });
    }
...
export function checkStructuralFailure(sections, addEvent) {
    ...
    if (overstressed) {
        if (state.structuralFailureTime === null) {
            ...
            addEvent(`SOBRECARGA ESTRUCTURAL: ${worst.name} (${(worst.utilization * 100).toFixed(0)}% del límite)`);
        }
        ...
        if (state.settings.structuralFailureMode === 'terminate') {
            ...
            if (elapsed >= getStructuralFailureGraceSeconds()) {
                addEvent('FALLO ESTRUCTURAL - VEHÍCULO PERDIDO');
