...
const FAILURE_COPY = {
    impact: {
        title: 'FALLO DE MISIÓN',
        message: 'Impacto contra el suelo — vehículo destruido.',
    },
    structural: {
        title: 'FALLO ESTRUCTURAL',
        message: 'Vehículo perdido — límites estructurales superados.',
    },
};
...
export function registerMissionFailure(reason) {
    ...
    if (restartBtn) {
        restartBtn.textContent = challenge ? 'REINICIAR DESDE LA PLATAFORMA' : 'REINICIAR';
    }
    if (altBtn) {
        altBtn.textContent = challenge ? 'IR AL HANGAR' : 'IR AL MENÚ';
    }
