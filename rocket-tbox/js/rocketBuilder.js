...
const SEGMENT_DEFS = [
    { index: 0, title: 'Etapa 1', pathPrefix: 'stages.0.', shortLabel: 'E1' },
    { index: 1, title: 'Etapa 2', pathPrefix: 'stages.1.', shortLabel: 'E2' },
    { index: 2, title: 'Carga útil', pathPrefix: 'payload.', shortLabel: 'CU' },
    { index: 3, title: 'Carenado', pathPrefix: 'fairing.', shortLabel: 'CN' },
];

const GLOBAL_PATHS = new Set(['fairingJettisonAlt', 'propellantDensity']);

export const ROCKET_BUILDER_SCHEMA = [
    { path: 'stages.0.dryMass', label: 'Masa seca (kg)', min: 5000, max: 50000, step: 100, tab: 'basic' },
    { path: 'stages.0.thrust', label: 'Empuje NM (N)', min: 1e6, max: 15e6, step: 100000, tab: 'basic' },
    { path: 'stages.0.thrustVac', label: 'Empuje vacío (N)', min: 1e6, max: 15e6, step: 100000, tab: 'basic' },
    { path: 'stages.0.diameter', label: 'Diámetro (m)', min: 2, max: 6, step: 0.1, tab: 'basic' },
    { path: 'stages.0.length', label: 'Longitud (m)', min: 20, max: 60, step: 1, tab: 'basic' },
    { path: 'stages.0.propellantMass', label: 'Propelente (kg)', min: 100000, max: 600000, step: 1000, tab: 'basic', locked: true },
    { path: 'stages.0.isp', label: 'Isp NM (s)', min: 250, max: 350, step: 1, tab: 'advanced' },
    { path: 'stages.0.ispVac', label: 'Isp vacío (s)', min: 280, max: 380, step: 1, tab: 'advanced' },
    { path: 'stages.0.tankLengthRatio', label: 'Ratio de tanque', min: 0.5, max: 0.95, step: 0.01, tab: 'advanced' },
    { path: 'stages.0.engineLength', label: 'Longitud del motor (m)', min: 1, max: 5, step: 0.1, tab: 'advanced' },
    { path: 'stages.0.dryMassEngineFraction', label: 'Fracción masa motor', min: 0.3, max: 0.8, step: 0.05, tab: 'advanced' },
    { path: 'stages.0.gimbalMaxAngle', label: 'Gimbal máx (°)', min: 1, max: 15, step: 0.5, tab: 'advanced' },
    { path: 'stages.0.gimbalRate', label: 'Vel. de gimbal (°/s)', min: 5, max: 30, step: 0.5, tab: 'advanced' },
    { path: 'stages.0.gimbalPoint', label: 'Punto de gimbal (m)', min: 0.2, max: 1.5, step: 0.1, tab: 'advanced' },
    { path: 'stages.1.dryMass', label: 'Masa seca (kg)', min: 1000, max: 8000, step: 100, tab: 'basic' },
    { path: 'stages.1.thrust', label: 'Empuje NM (N)', min: 100000, max: 2e6, step: 10000, tab: 'basic' },
    { path: 'stages.1.thrustVac', label: 'Empuje vacío (N)', min: 100000, max: 2e6, step: 10000, tab: 'basic' },
    { path: 'stages.1.diameter', label: 'Diámetro (m)', min: 2, max: 6, step: 0.1, tab: 'basic' },
    { path: 'stages.1.length', label: 'Longitud (m)', min: 5, max: 25, step: 0.5, tab: 'basic' },
    { path: 'stages.1.propellantMass', label: 'Propelente (kg)', min: 20000, max: 150000, step: 500, tab: 'basic', locked: true },
    { path: 'stages.1.isp', label: 'Isp NM (s)', min: 300, max: 360, step: 1, tab: 'advanced' },
    { path: 'stages.1.ispVac', label: 'Isp vacío (s)', min: 320, max: 380, step: 1, tab: 'advanced' },
    { path: 'stages.1.tankLengthRatio', label: 'Ratio de tanque', min: 0.5, max: 0.95, step: 0.01, tab: 'advanced' },
    { path: 'stages.1.engineLength', label: 'Longitud del motor (m)', min: 0.5, max: 4, step: 0.1, tab: 'advanced' },
    { path: 'stages.1.dryMassEngineFraction', label: 'Fracción masa motor', min: 0.3, max: 0.8, step: 0.05, tab: 'advanced' },
    { path: 'stages.1.gimbalMaxAngle', label: 'Gimbal máx (°)', min: 1, max: 15, step: 0.5, tab: 'advanced' },
    { path: 'stages.1.gimbalRate', label: 'Vel. de gimbal (°/s)', min: 5, max: 25, step: 0.5, tab: 'advanced' },
    { path: 'stages.1.gimbalPoint', label: 'Punto de gimbal (m)', min: 0.2, max: 1, step: 0.1, tab: 'advanced' },
    { path: 'payload.mass', label: 'Masa (kg)', min: 1000, max: 30000, step: 500, tab: 'basic' },
    { path: 'payload.length', label: 'Longitud (m)', min: 2, max: 15, step: 0.5, tab: 'basic' },
    { path: 'payload.diameter', label: 'Diámetro (m)', min: 2, max: 6, step: 0.1, tab: 'basic' },
    { path: 'fairing.mass', label: 'Masa (kg)', min: 500, max: 3000, step: 50, tab: 'basic' },
    { path: 'fairing.length', label: 'Longitud (m)', min: 2, max: 10, step: 0.5, tab: 'basic' },
    { path: 'fairing.diameter', label: 'Diámetro (m)', min: 2, max: 6, step: 0.1, tab: 'basic' },
    { path: 'fairingJettisonAlt', label: 'Alt. eyección carenado (m)', min: 80000, max: 150000, step: 1000, tab: 'advanced' },
    { path: 'propellantDensity', label: 'Densidad del propelente (kg/m³)', min: 800, max: 1200, step: 10, tab: 'advanced' },
];
...
function buildFormRows(container, entries, config, rowClassName = 'rocket-builder-row') {
    ...
    entries.forEach((entry) => {
        ...
        if (entry.locked) {
            input.disabled = true;
            input.readOnly = true;
            input.title = 'Calculado a partir del volumen del tanque (longitud × diámetro)';
        }
...
function openSegmentPopup(segmentIndex, clientX, clientY, tab = 'basic') {
    ...
    if (title) title.textContent = SEGMENT_DEFS[segmentIndex]?.title || 'Sección';
