...
function setupBurnButton(buttonId, burnMode) {
    ...
    btn.addEventListener('mousedown', (e) => {
        ...
        if (pitchProgramComplete && altitude > 150000) {
            ...
            addEvent(`Quemado ${burnStartNames[burnMode] || burnMode.toUpperCase()} iniciado`);
        }
    });
...
export function initInput() {
    ...
    document.getElementById('reset-btn').addEventListener('click', () => {
        ...
        launchBtnEl.textContent = (state.gameMode === 'orbital') ? 'INICIAR' : 'LANZAR';
    ...
    document.getElementById('zoom-auto-btn').addEventListener('click', () => {
        ...
        document.getElementById('zoom-auto-btn').textContent = state.autoZoom ? 'ZOOM AUTO' : 'ZOOM MANUAL';
        if (!state.autoZoom) {
    ...
    document.getElementById('camera-btn').addEventListener('click', () => {
        ...
        document.getElementById('camera-btn').textContent = state.cameraMode === 'rocket' ? 'SEGUIR COHETE' : 'CENTRAR TIERRA';
    ...
    function updateCameraBtnLabels() {
        ...
        if (aBtn) aBtn.textContent = isRocket ? 'SEGUIR COHETE' : 'CENTRAR TIERRA';
        if (bBtn) {
            bBtn.textContent = isRocket ? 'CÁM. COHETE' : 'CÁM. ORBITAL';
    ...
    document.getElementById('refuel-btn').addEventListener('click', () => {
        if (state.currentStage < getRocketConfig().stages.length) {
            ...
            addEvent(`Repostado: +${refuelAmount} kg de propelente`);
        }
    });
    ...
    function updateControlModeUI() {
        ...
        if (controlModeDescription) {
            controlModeDescription.textContent = isGimbalMode 
                ? 'Controla el ángulo de gimbal directamente. La velocidad de giro depende del empuje.'
                : 'Controla el ángulo de cabeceo objetivo. El gimbal se ajusta automáticamente.';
        }
        ...
        if (controlTitle) {
            controlTitle.textContent = isGimbalMode ? 'CONTROL MANUAL DE GIMBAL' : 'CONTROL MANUAL DE CABECEO';
        }
        if (gimbalDisplay) {
            gimbalDisplay.style.display = isGimbalMode ? 'block' : 'none';
        }
        if (pitchUpBtn) {
            pitchUpBtn.innerHTML = isGimbalMode ? '↑ SUBIR GIMBAL<br>(W / ↑)' : '↑ SUBIR CABECEO<br>(W / ↑)';
        }
        if (pitchDownBtn) {
            pitchDownBtn.innerHTML = isGimbalMode ? '↓ BAJAR GIMBAL<br>(S / ↓)' : '↓ BAJAR CABECEO<br>(S / ↓)';
        }
