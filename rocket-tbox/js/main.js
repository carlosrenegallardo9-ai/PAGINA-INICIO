...
function update(dt) {
    ...
    if (r < EARTH_RADIUS && state.time > 1 && state.engineOn) {
        addEvent("FALLO DE MISIÓN - Impacto contra el suelo");
            ...
            if (state.gameMode === 'orbital') {
                ...
                if (shouldLogGuidanceEvents && step === 0 && guidance.phase === 'vacuum-guidance' && guidance.debug && guidance.debug.reason) {
                    ...
                    if (useDirectAscent && 
                        ...
                        addEvent(`Quemado de ascenso directo - elevando periastro al objetivo`);
                    }
                    ...
                    // Only announce if more than 25 minutes have passed (avoids false positives during ascent)
                        ...
                        addEvent(`Inicio de quemado de circularización (Δv: ${(deltaV/1000).toFixed(1)} km/s, ${burnTime.toFixed(1)}s)`);
                    }
                    ...
                    if (reason.includes('Starting retrograde burn') && 
                        ...
                        addEvent(`Inicio de quemado retrógrado (Δv: ${(deltaV/1000).toFixed(1)} km/s, ${burnTime.toFixed(1)}s)`);
                    }
                }
            }
        } else {
    ...
    if (state.currentStage < rocketConfig.stages.length && 
        ...
        if (state.currentStage === 0) {
            ...
            if (state.burnMode) {
                ...
                addEvent(`Quemado ${burnNames[state.burnMode]} finalizado - sin propelente (${duration}s)`);
                state.burnMode = null;
    ...
    if (state.burnMode && !state.engineOn && state.burnStartTime !== null) {
        ...
        addEvent(`Quemado ${burnNames[state.burnMode]} finalizado (${duration}s)`);
        state.burnMode = null;
        state.burnStartTime = null;
    }
    ...
    if (!state.fairingJettisoned && altitude > rocketConfig.fairingJettisonAlt) {
        ...
        addEvent("Eyección del carenado");
    }
    ...
    const shouldLogGuidanceEvents = state.gameMode === 'guided' || state.gameMode === 'cubic';
    if (shouldLogGuidanceEvents && state.time >= 10.0 && !state.events.some(e => e.text.includes("Giro gravitacional iniciado"))) {
        addEvent("Giro gravitacional iniciado");
    }
    if (shouldLogGuidanceEvents && state.time >= 13.0 && !state.events.some(e => e.text.includes("Giro gravitacional activo"))) {
        addEvent("Giro gravitacional activo - empuje prógrado");
    }
    
    if (altitude >= KARMAN_LINE && !state.events.some(e => e.text.includes("Kármán"))) {
        addEvent("Línea de Kármán - ¡ESPACIO!");
    }
...
function updateCurrentModeDisplay() {
    ...
    if (modeDisplay) {
        if (state.gameMode === 'manual') {
            modeDisplay.textContent = 'Modo actual: Control Manual';
        } else if (state.gameMode === 'guided') {
            modeDisplay.textContent = `Modo actual: Lanzamiento Guiado (objetivo ${(state.targetAltitude / 1000).toFixed(0)}km)`;
        } else if (state.gameMode === 'orbital') {
            modeDisplay.textContent = `Modo actual: Orbital (${(state.orbitalSpawnAltitude / 1000).toFixed(0)}km)`;
        } else if (state.gameMode === 'cubic') {
            modeDisplay.textContent = `Modo actual: Guiado Cúbico (objetivo ${(state.targetAltitude / 1000).toFixed(0)}km)`;
        } else {
            modeDisplay.textContent = 'Ninguna misión seleccionada';
        }
    }
}
...
function startMission(mode, options = {}) {
    ...
    if (launchBtn) {
        ...
        launchBtn.textContent = (mode === 'orbital') ? 'INICIAR' : 'LANZAR';
    }
    if (pauseBtn) {
...
function updateUIForMode() {
    ...
    if (launchBtn && state.gameMode !== null) {
        launchBtn.style.display = 'inline-block';
        launchBtn.textContent = (state.gameMode === 'orbital') ? 'INICIAR' : 'LANZAR';
    }
}
...
function updateGuidanceModeAvailability() {
    ...
    if (custom) {
        ...
        if (selected && selected.dataset.mode === 'cubic') {
            ...
            if (launchBtn) { launchBtn.disabled = true; launchBtn.textContent = 'SELECCIONA UN MODO'; }
        }
    }
}
...
function updateChallengeCardUI() {
    ...
    if (label) label.textContent = challenge ? 'SALIR DEL DESAFÍO' : 'DESAFÍO';
    if (sub) sub.textContent = challenge ? 'Restaurar valores por defecto' : 'Dificultad extrema';
    if (badgeMain) badgeMain.textContent = challenge ? 'SALIR' : 'DIFÍCIL';
    if (badgeSub) badgeSub.textContent = challenge ? '¿Estás seguro?' : 'Diseña y vuela';
}
...
function exitChallengeFromMenu() {
    ...
    if (menuLaunchBtn) {
        ...
        menuLaunchBtn.textContent = 'SELECCIONA UN MODO';
    }
...
function preselectChallengeManualMode() {
    ...
    if (menuLaunchBtn) {
        ...
        menuLaunchBtn.textContent = 'LANZAR — CONTROL MANUAL';
    }
    ...
    if (challenge) {
        ...
        if (selected && selected.dataset.mode === 'orbital') {
            ...
            if (launchBtn) {
                ...
                launchBtn.textContent = 'SELECCIONA UN MODO';
            }
        }
    }
}
...
function initMenu() {
    ...
    const launchLabels = {
        manual: 'LANZAR — CONTROL MANUAL',
        guided: 'LANZAR — GUIADO',
        cubic:  'LANZAR — GUIADO CÚBICO',
        orbital: 'APARECER EN ÓRBITA',
    };
    ...
    modeCards.forEach(card => {
        card.addEventListener('click', () => {
            ...
            if (launchBtn) {
                launchBtn.disabled = false;
                launchBtn.textContent = launchLabels[mode] || 'LANZAR';
            }
        });
    });
    ...
    if (controlModeRow) {
        controlModeRow.addEventListener('click', () => {
            ...
            if (controlModeVal) {
                controlModeVal.textContent = isGimbal ? 'VEL. DE GIRO ›' : 'GIMBAL ›';
            }
        });
    }
...
function syncSettingsUI() {
    ...
    const structToggle   = document.getElementById('struct-fail-toggle');
    if (controlModeVal) {
        controlModeVal.textContent = state.settings.controlMode === 'gimbal' ? 'GIMBAL ›' : 'VEL. DE GIRO ›';
    }
